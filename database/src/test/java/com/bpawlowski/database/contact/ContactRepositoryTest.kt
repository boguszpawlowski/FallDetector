...
class ContactRepositoryTest {
    ...
    fun `When there is contact with same mobile then return failure`() = runBlocking {
        ...
        assert((result as? com.bpawlowski.domain.domain.FallDetectorResult.Result.Failure)?.error == FallDetectorException.MobileAlreadyExisting(normalContact.mobile))
    }
    ...
    fun `When ice contact exists then don't allow to add another`() = runBlocking {
        ...
        verify(contactDao, never()).insert(any())
        assert((result as? com.bpawlowski.domain.domain.FallDetectorResult.Result.Failure)?.error == FallDetectorException.IceAlreadyExistsException)
    }
    ...
    fun `When contact is valid, add it to database`() = runBlocking {
        ...
        assert((result as? com.bpawlowski.domain.domain.FallDetectorResult.Result.Success)?.data == 1L)
    }
    ...
    fun `When iceContact has same id then allow to update it`() = runBlocking {
        ...
        assert((result as? com.bpawlowski.domain.domain.FallDetectorResult.Result.Success)?.data == Unit)
    }
