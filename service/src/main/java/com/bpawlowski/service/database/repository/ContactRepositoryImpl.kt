...
import com.bpawlowski.service.util.sortedByDescending
...
internal class ContactRepositoryImpl(
	...
	override suspend fun updateContact(contact: Contact): FallDetectorResult<Unit> = withContext(Dispatchers.IO) {
		when {
			...
			contactDao.getContactByMobile(contact.mobile) != null -> failure(FallDetectorException.MobileAlreadyExisting(contact.mobile))
			else -> {
